...
**重要：流程分阶段确认，HTML 预览生成后等待用户反馈再继续。草图阶段不设确认。**
...
流程分阶段确认，用户在 HTML 预览节点有机会修改，确认满意后再继续下一步。草图生成后自动进入 HTML 生成。
...
草图生成后，**直接进入 Step 4 生成画板 HTML 预览**，无需等待用户确认。
